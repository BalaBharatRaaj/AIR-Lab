...
<?php
	...
	if(isset($_SESSION['table'])){
		if($_SESSION['table'] == "Moudetails"){
			echo '<h1 style="margin-bottom: 80px">MEMORANDUM OF UNDERSTANDING (MoU) TABLE</h1>';
		...
		elseif($_SESSION['table'] == "private"){
			echo '<h1 style="margin-bottom: 80px">PRIVATE CLOUD DETAILS TABLE</h1>';
		...
		elseif($_SESSION['table'] == "funds"){
			echo '<h1 style="margin-bottom: 80px">FUNDS RECEVIED TABLE</h1>';
		...
		elseif($_SESSION['table'] == "datacenter"){
			echo '<h1 style="margin-bottom: 80px">CLOUD DATA CENTER TABLE</h1>';
		...
		elseif($_SESSION['table'] == "serverconfig"){
			echo '<h1 style="margin-bottom: 80px">AIR LAB SERVER CONFIGURATION TABLE</h1>';
		...
		elseif($_SESSION['table'] == "security"){
			echo '<h1 style="margin-bottom: 80px">DEVICES WITH LOGS AND USER COUNT TABLE</h1>';
			...
			echo '</table>';
			echo '<h1 style="margin-bottom: 80px; margin-top: 80px">DEVICES WITH QUANTITY TABLE</h1>';
		...
		elseif($_SESSION['table'] == "research"){
			echo '<h1 style="margin-bottom: 80px">RESEARCH TABLE</h1>';
		...
		elseif($_SESSION['table'] == "project"){
			echo '<h1 style="margin-bottom: 80px">PROJECT DETAILS TABLE</h1>';
		...
		elseif($_SESSION['table'] == "faculty"){
			...
			echo('<table class="table table-bordered table-striped table-hover" border="2">'."\n");
			echo '<tr><th scope="col">Faculty ID</th><th scope="col">Faculty Name</th></tr>';
			while ( $row = $stmt->fetch(PDO::FETCH_ASSOC) ) {
			    echo "<tr><td>";
		...
		elseif($_SESSION['table'] == "student"){
			...
			echo('<table class="table table-bordered table-striped table-hover" border="2">'."\n");
			echo '<tr><th scope="col">Roll number</th><th scope="col">Student Name</th></tr>';
			while ( $row = $stmt->fetch(PDO::FETCH_ASSOC) ) {
			    echo "<tr><td>";
		...
		elseif($_SESSION['table'] == "concept"){
			...
			echo('<table class="table table-bordered table-striped table-hover" border="2">'."\n");
			echo '<tr><th scope="col">Field</th><th scope="col">Title</th></tr>';
			while ( $row = $stmt->fetch(PDO::FETCH_ASSOC) ) {
			    echo "<tr><td>";
